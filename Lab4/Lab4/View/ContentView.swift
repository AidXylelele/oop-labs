...
//  Lab4
//
//  Created by Владислав on 08.11.2022.
//
...
struct ContentView: View {
    ...
    @State private var colors: [Color] = [ .black, .black, .black, .black, .black, .black, .black ]
        ...
        VStack {
            ...
            HStack {
                    ...
                    VStack {
                        Image(systemName: "scribble").font(.system(size: 30))
                            .onTapGesture {
                                ...
                                flag = "line"
                                print(label)
                                colors = [.black, .blue, .black, .black, .black, .black, .black ]
                            }
                            .onLongPressGesture(minimumDuration: 0.5) {
                    ...
                    VStack {
                        Image(systemName: "line.diagonal").font(.system(size: 30))
                            .onTapGesture {
                                ...
                                flag = "straight line"
                                print(label)
                                colors = [.black, .black, .blue, .black, .black, .black, .black]
                            }
                            .onLongPressGesture(minimumDuration: 0.5) {
                    ...
                    VStack {
                        Image(systemName: "oval").font(.system(size: 30))
                            .onTapGesture {
                                ...
                                flag = "ellipse"
                                print(label)
                                colors = [.black, .black, .black, .blue, .black, .black, .black]
                            }
                            .onLongPressGesture(minimumDuration: 0.5) {
                    ...
                    VStack {
                        Image(systemName: "square").font(.system(size: 30))
                            .onTapGesture {
                                ...
                                flag = "rectangle"
                                print(label)
                                colors = [.black, .black, .black, .black, .blue, .black, .black]
                            }
                            .onLongPressGesture(minimumDuration: 0.5) {
                ...
                Button (action: {}) {
                    Image(systemName: "link").font(.system(size: 30))
                        .onTapGesture {
                            ...
                            flag = "lineOO"
                            print(label)
                            colors = [.black, .black, .black, .black, .black, .blue, .black]
                        }
                        .onLongPressGesture(minimumDuration: 0.5) {
                ...
                Menu {
                    ...
                    Button("Еллипс") {
                        ...
                        flag = "ellipse"
                        print(label)
                        colors = [.black, .black, .gray, .black, .black, .black, .black]
                    }
            ...
            .gesture(DragGesture(minimumDistance: 0, coordinateSpace: .local).onChanged({ value in
                if flag == "point" {
                    ...
                    if value.translation.width + value.translation.height == 0 {
                      let firstPoint = value.location
                      figures.array.append(LineOO(points: [firstPoint], color: selectedColor, lineWidth: selectedWidth, width: 0, height: 0, backgroundColor: .clear))
                    } else {
                      let index = figures.array.count - 1
                      if figures.array[index].points.count == 2 {
                        figures.objectWillChange.send()
                  ...
                  if value.translation.width + value.translation.height == 0 {
                    let firstPoint = value.startLocation
                    figures.array.append(Cube(points: [firstPoint],color: selectedColor, lineWidth: selectedWidth, width: 0, height: 0, backgroundColor: .clear))
                  } else {
                    let index = figures.array.count - 1
                    figures.objectWillChange.send()
                    figures.array[index].width = value.translation.width
